...
export function getCountryTranslation(countryName: string, translations: TranslationData): string {
	const normalized = normalizeCountry(countryName, translations.aliases);
	return translations.countries[normalized] || countryName;
}

export function getCityTranslation(cityName: string, translations: TranslationData): string {
